...
class Evaluator(object):
    def __init__(self, allowed_train_time):
        ...
        trainset = torchvision.datasets.CIFAR10(root='../GAcode/data', train=True,
                                                download=True, transform=transform)
        self._trainloader = torch.utils.data.DataLoader(trainset, batch_size=64,
                                                  shuffle=True, num_workers=6)

        testset = torchvision.datasets.CIFAR10(root='../GAcode/data', train=False,
                                               download=True, transform=transform)
        self._testloader = torch.utils.data.DataLoader(testset, batch_size=64,
    ...
    def _indiv_evaluator(self, filename):
        ...
        for epoch in range(30):
            ...
            print '[E: %d] correct: %.2f, time: %.2f' % (epoch, accuracy, inf_time)
...
if __name__ == '__main__':
    E = Evaluator(2*60)
    E.evaluate('../GAcode/mutation/', ['mod_ACGAN_D_model.py'])